...
Each project that applies the plugin produces its own `+build/gl-code-quality-report.json+`. To get a single file for GitLab CI, the plugin also registers an `+aggregateGitLabCodeQualityReport+` task on every project — it merges the reports of that project and **all of its subprojects** (recursive) into one JSON file.

The simplest setup is to apply the plugin only at the **root project** and let it propagate:
...
gitlabCodeQuality {
    applyToSubprojects.set(true)
}
----

That is equivalent to writing `+subprojects { apply(plugin = "io.github.dnalchemist.gitlab-code-quality") }+` by hand — use whichever style your build prefers. Each subproject still needs its own SpotBugs / Checkstyle configuration.

Run the aggregator and point GitLab CI at its output (`+build/gl-code-quality-aggregate.json+` of the root project by default):
...
build:
  ...
  image: ...
  script:
    - ./gradlew check aggregateGitLabCodeQualityReport
  artifacts:
    reports:
      codequality:
        - build/gl-code-quality-aggregate.json
----

The aggregator's inputs and output paths are configurable like any other Gradle task:

[source,kotlin]
----
import io.github.dnalchemist.gitlab.codequality.AggregateGitLabCodeQualityReportTask

tasks.named<AggregateGitLabCodeQualityReportTask>("aggregateGitLabCodeQualityReport") {
    // Add reports from external locations:
    inputReports.from(file("legacy-reports/another-gl-code-quality-report.json"))
    outputFile.set(layout.buildDirectory.file("reports/gitlab/code-quality.json"))
}
----
